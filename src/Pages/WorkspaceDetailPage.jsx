...
import {
  AppBar, Toolbar, Typography, Box, List, ListItem, ListItemIcon, ListItemText, Tabs, Tab, Avatar, TextField, Button, Paper, Divider, IconButton, InputAdornment, Stack, Grid, Card, CardContent
} from '@mui/material';
...
const WorkspaceDetailPage = () => {
  ...
  return (
    <Box sx={{ display: 'flex', background: '#f7f8fa', position: 'relative' }}>
      {/* Sidebar */}
      <Box
        sx={{
          width: sidebarWidth,
      ...
      >
        <List sx={{ mt: 2 }}>
          {navItems.map((item) => (
            <ListItem button key={item.label} sx={{ borderRadius: 1, mb: 0.5, px: 2, backgroundColor: sideBarItem === item.label ? '#f0f0f0' : 'transparent', cursor:'pointer' }} onClick={() => setSideBarItem(item.label)}>
          ...
          <Box sx={{ mt: 1, ml: 4 }}>
            {(sideBarItem === 'Options' || sideBarItem === 'Users' || sideBarItem === 'Features' || sideBarItem === 'Settings' || sideBarItem === 'Activity') && navItems[6]?.subMenu?.map((item) => (
            <ListItem button key={item.label} sx={{ borderRadius: 1, mb: 0.5, px: 2, backgroundColor: sideBarItem === item.label ? '#f0f0f0' : 'transparent', cursor:'pointer' }} onClick={() => setSideBarItem(item.label)}>
              <ListItemIcon sx={{ minWidth: 36 }}>{item.icon}</ListItemIcon>
              <ListItemText primary={item.label} primaryTypographyProps={{ fontSize: 15 }} />
              </ListItem>
      ...
      {/* Main Content */}
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative', maxHeight:'85vh', overflowY:'auto', width:'100%', scrollbarWidth:'none', scrollbarColor:'#562583 #f7f8fa' }}>
            ...
            <>
              ...
              <Paper sx={{ mb: 2, p: 2, borderRadius: 2, boxShadow: 'none', border: '1px solid #eee' }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 1, gap: 1 }}>
                  <Button size="small" sx={{ textTransform: 'none' }}>Cancel</Button>
                  <Button size="small" variant="contained" sx={{ textTransform: 'none' }} endIcon={<SendIcon />}>Post</Button>
                </Box>
              </Paper>
                ...
                {posts.map(post => (
                  <Paper key={post.id} sx={{ p: 2, borderRadius: 2, boxShadow: 'none', border: '1px solid #eee' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                      ...
                      <Typography variant="subtitle2" sx={{ fontWeight: 500 }}>{post.user.name}</Typography>
                      <Typography variant="caption" sx={{ ml: 1, color: 'text.secondary' }}>{post.time} · {post.workspace}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                      {post.fileType === 'excel' && <InsertDriveFileIcon sx={{ color: 'green', mr: 1 }} />}
                      {post.fileType === 'word' && <InsertDriveFileIcon sx={{ color: 'blue', mr: 1 }} />}
                      <Typography variant="body2" sx={{ fontWeight: 500 }}>{post.content}</Typography>
                    </Box>
                    {post.comments.map(comment => (
                      <Box key={comment.id} sx={{ ml: 5, mt: 1, p: 1, background: '#f5f6fa', borderRadius: 1 }}>
                        <Typography variant="body2" sx={{ fontWeight: 500 }}>{comment.user.name}</Typography>
